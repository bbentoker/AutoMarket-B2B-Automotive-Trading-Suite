 ...
 * 
 * This script fetches and uploads images for Swiss ListingSiteA adverts (CHF currency).
...
async function processSwissAdvertImage(advert) {
  try {
    console.log(`[SWISS_FETCHER] 📋 Processing Swiss advert ${advert.listingsitea_id} (${advert.make} ${advert.model})`);
    
    // Check if we already have a MinIO URL (starts with our MinIO domain)
    if (advert.image_url && advert.image_url.includes('s3.automarket.example.com')) {
      console.log(`[SWISS_FETCHER] ✅ Advert ${advert.listingsitea_id} already has MinIO image: ${advert.image_url}`);
      return true;
    }
    
    // Check if we have an original image URL to work with
    if (!advert.image_url || advert.image_url.trim() === '') {
      console.log(`[SWISS_FETCHER] ⚠️ No image URL found for Swiss advert ${advert.listingsitea_id}`);
      return false;
    }
    ...
    console.error(`[SWISS_FETCHER] ❌ Error processing Swiss advert ${advert.listingsitea_id}:`, error.message);
    return false;
  }
}
  ...
  try {
    ...
    const whereConditions = {
      [Op.and]: [
        ...
        {
          [Op.or]: [
            { image_url: { [Op.notLike]: '%s3.automarket.example.com%' } }, // Not already MinIO URL
    ...
    for (let i = 0; i < adverts.length; i++) {
      ...
      console.log(`[SWISS_FETCHER] 📋 Processing ${i + 1}/${adverts.length}: ${advert.listingsitea_id}`);
      ...
      try {
        ...
        if (success) {
          successful++;
          console.log(`[SWISS_FETCHER] ✅ Successfully processed ${advert.listingsitea_id}`);
        } else {
          failed++;
          console.log(`[SWISS_FETCHER] ❌ Failed to process ${advert.listingsitea_id}`);
        }
...
async function main() {
  try {
    ...
    console.log('🚀 Starting Swiss ListingSiteA Image Fetcher...');
