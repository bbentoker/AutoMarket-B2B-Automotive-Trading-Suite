...
const { ListingSiteAInventory } = require('../../models');
...
async function main() {
    ...
    logger.info('[SCRAPER] 🚀 Starting ListingSiteA scraper...');
    ...
    try {
        ...
        // sort the users by listingsitea_adverts_count count desc
        users.sort((a, b) => {
            return b.listingsitea_adverts_count - a.listingsitea_adverts_count;
        });
        ...
        if (process.env.DEBUG === 'true') {
            ...
            logger.info(`[SCRAPER] 🎯 Debug filter: Swiss ListingSiteA.ch URLs only`);
            logger.info(`[SCRAPER] 📋 Filtered users: [${users.map(u => `${u.id} (${u.listingsitea_url})`).join(', ')}]`);
        }
...
async function scrapeUsersListings(user, control) {
    ...
    const isInitialRun = user.listingsitea_adverts_count === 0;
    
    logger.info(`[SCRAPER] 🚀 Starting scraping session for user ${user.id} (${user.company_name || 'Unknown'})`);
    logger.info(`[SCRAPER] 🔗 URL: ${user.listingsitea_url}`);
    logger.info(`[SCRAPER] ⏰ Session start time: ${userStartTime.toLocaleString()}`);
    
    if (isInitialRun) {
        logger.info(`[SCRAPER] 🔄 INITIAL RUN detected for user ${user.id} (listingsitea_adverts_count: ${user.listingsitea_adverts_count})`);
        logger.info(`[SCRAPER] 📝 All new listings for this user will be marked as initial run listings`);
    } else {
        logger.info(`[SCRAPER] ✅ Regular run for user ${user.id} (listingsitea_adverts_count: ${user.listingsitea_adverts_count})`);
    }
