...
const { getHttpsAgent, getRequestAgent } = require('./listingsiteaApi');
...
async function handleAdvertNotFound(listingsiteaId) {
  try {
    logger.warn(`[CHECKER] 🚫 Advert ${listingsiteaId} not available, marking ALL with same listingsitea_id as inactive...`);

    const adverts = await Advert.findAll({ where: { listingsitea_id: listingsiteaId } });
    if (!adverts || adverts.length === 0) {
      logger.error(`[CHECKER] ❌ No adverts found in database for listingsitea_id ${listingsiteaId}`);
      return;
    }
    ...
    for (const advert of adverts) {
      try {
        ...
        await advert.save();
        logger.info(`[CHECKER] ✅ Advert id=${advert.id} (listingsitea_id=${listingsiteaId}) marked as inactive`);
      } catch (rowErr) {
        logger.error(`[CHECKER] ❌ Failed to mark advert id=${advert.id} inactive:`, rowErr.message);
      }
    }
  } catch (error) {
    logger.error(`[CHECKER] ❌ Error handling not-available adverts for listingsitea_id ${listingsiteaId}:`, error.message);
    // Fallback: best-effort bulk deactivate without dates/sell_time calc
    try {
      await Advert.update({ is_active: false }, { where: { listingsitea_id: listingsiteaId } });
      logger.info(`[CHECKER] ✅ Fallback: All adverts with listingsitea_id=${listingsiteaId} marked inactive`);
    } catch (fallbackError) {
      logger.error(`[CHECKER] ❌ Fallback error bulk-updating listingsitea_id ${listingsiteaId}:`, fallbackError.message);
    }
  }
}

async function checkAdvertAvailability(listingsiteaId) {
  const baseUrl = process.env.LISTING_SITE_A_URL || 'https://www.listingsitea.example.com';
  const advertUrl = `${baseUrl}/offers/${listingsiteaId}`;
  const response = await axios.get(advertUrl, {
    httpsAgent: getRequestAgent(),
    headers: {
...
async function processAdvertsSequentially(adverts) {
  ...
  for (let i = 0; i < adverts.length; i++) {
    ...
    logger.info(`[CHECKER] 📋 Processing advert ${i + 1}/${adverts.length}: ${advert.listingsitea_id}`);

    // Validate UUID format before processing
    if (!advert.listingsitea_id || !uuidRegex.test(advert.listingsitea_id)) {
      logger.error(`[CHECKER] ❌ Invalid UUID format for advert listingsitea_id: ${advert.listingsitea_id} - skipping processing`);
      errorCount++;
      continue;
    }
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
      try {
        logger.info(`[CHECKER] 🔎 Checking availability for advert: ${advert.listingsitea_id} (attempt ${attempt}/3)`);
        const isAvailable = await checkAdvertAvailability(advert.listingsitea_id);
        if (!isAvailable) throw new Error('Listing elements not found on page');
        logger.info(`[CHECKER] ✅ Listing appears available for advert: ${advert.listingsitea_id} on attempt ${attempt}`);
        successCount++;
        processed = true;
        break;
      } catch (error) {
        lastError = error;
        logger.warn(`[CHECKER] ⚠️ Attempt ${attempt}/3 failed for advert ${advert.listingsitea_id}:`, error.message);
        if (attempt === 3) {
          logger.error(`[CHECKER] ❌ All 3 attempts failed for advert ${advert.listingsitea_id}:`, error.message);
          await handleAdvertNotFound(advert.listingsitea_id);
          inactiveCount++;
          processed = true;
        } else {
          logger.info(`[CHECKER] ⏳ Waiting 1 second before retry for advert ${advert.listingsitea_id}...`);
...
async function checkListings() {
  ...
  try {
    const activeAdverts = await Advert.findAll({
      ...
      attributes: ['listingsitea_id', 'seller_id']
    });
...
async function checkListingsAcrossUsers() {
  ...
  // DEBUG MODE: Filter users with Swiss ListingSiteA.ch URLs if DEBUG=true
  if (process.env.DEBUG === 'true') {
    const originalCount = users.length;
    users = users.filter(user => user.listingsitea_url && user.listingsitea_url.includes('listingsitea.ch'));
    logger.info(`[CHECKER] 🐛 DEBUG MODE ENABLED: Filtered to ${users.length} users from ${originalCount} total users`);
    logger.info(`[CHECKER] 🎯 Debug filter: Swiss ListingSiteA.ch URLs only`);
    logger.info(`[CHECKER] 📋 Filtered users: [${users.map(u => `${u.id} (${u.listingsitea_url})`).join(', ')}]`);
  }
