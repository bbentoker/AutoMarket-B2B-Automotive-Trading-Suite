 ...
 * @param {Object} user - User object with listingsitea_url
...
async function checkSwissListingsIndividually(user) {
  ...
  try {
    ...
    if (process.env.DEBUG === 'true') {
      ...
      writeDebugLog(debugLogFilePath, `🇨🇭 Starting individual Swiss listings check for user ${user.id}: ${user.listingsitea_url}`);
    }
    
    logger.info(`[CHECKER] 🇨🇭 Starting individual Swiss listings check for user ${user.id}: ${user.listingsitea_url}`);
    ...
    const allAdverts = await Advert.findAll({
      ...
      attributes: ['id', 'listingsitea_id', 'make', 'model', 'price', 'created_at', 'is_active']
    });
    ...
    const activeIds = `💾 Active advert IDs: [${activeAdverts.map(advert => advert.listingsitea_id).sort().join(', ')}]`;
    logger.info('[CHECKER] ' + activeIds);
    writeDebugLog(debugLogFilePath, activeIds);
    
    const inactiveIds = `💾 Inactive advert IDs: [${inactiveAdverts.map(advert => advert.listingsitea_id).sort().join(', ')}]`;
    ...
    for (const advert of allAdverts) {
      try {
        const checkMsg = `🔍 Checking advert ${advert.listingsitea_id} individually...`;
        logger.info('[CHECKER] ' + checkMsg);
        writeDebugLog(debugLogFilePath, checkMsg);
        
        const listingData = await fetchSwissListingById(advert.listingsitea_id, debugLogFilePath);
        
        // Detailed logging for debugging false negatives
        const fetchResult = `🔍 [DEBUG] Advert ${advert.listingsitea_id} fetch result: ${listingData ? 'SUCCESS' : 'FAILED'}`;
        logger.info('[CHECKER] ' + fetchResult);
        writeDebugLog(debugLogFilePath, fetchResult);
        
        if (listingData) {
          const dataKeys = `🔍 [DEBUG] Advert ${advert.listingsitea_id} data keys: [${Object.keys(listingData).join(', ')}]`;
          logger.info('[CHECKER] ' + dataKeys);
          writeDebugLog(debugLogFilePath, dataKeys);
          
          const title = `🔍 [DEBUG] Advert ${advert.listingsitea_id} title: ${listingData.title || 'N/A'}`;
          logger.info('[CHECKER] ' + title);
          writeDebugLog(debugLogFilePath, title);
          
          const price = `🔍 [DEBUG] Advert ${advert.listingsitea_id} price: ${listingData.price?.amount || 'N/A'} ${listingData.price?.currency || ''}`;
          logger.info('[CHECKER] ' + price);
          writeDebugLog(debugLogFilePath, price);
          
          const makeModel = `🔍 [DEBUG] Advert ${advert.listingsitea_id} make/model: ${listingData.vehicle?.make || listingData.make || 'N/A'} ${listingData.vehicle?.model || listingData.model || 'N/A'}`;
          logger.info('[CHECKER] ' + makeModel);
          writeDebugLog(debugLogFilePath, makeModel);
        } else {
          const url = `🔍 [DEBUG] Advert ${advert.listingsitea_id} URL: https://www.listingsitea.ch/de/d/${advert.listingsitea_id}`;
          logger.warn('[CHECKER] ' + url);
          writeDebugLog(debugLogFilePath, url);
          
          const manualCheck = `🔍 [DEBUG] Advert ${advert.listingsitea_id} - PLEASE MANUALLY CHECK THIS URL TO VERIFY IF IT'S ACTUALLY SOLD`;
        ...
        if (listingData) {
          ...
          if (advert.is_active) {
            ...
            results.stillAvailable.push({
              listingsitea_id: advert.listingsitea_id,
              make: advert.make,
              model: advert.model,
              price: advert.price
            });
            const stillAvailable = `✅ [Swiss Individual] Advert ${advert.listingsitea_id} (${advert.make} ${advert.model}) still available`;
            ...
            const reactivated = `🔄 [Swiss Individual] Advert ${advert.listingsitea_id} (${advert.make} ${advert.model}) reactivated - back online!`;
          ...
          if (advert.is_active) {
            ...
            const markingInactive = `🚨 [Swiss Individual] MARKING ADVERT AS INACTIVE: ${advert.listingsitea_id} (${advert.make} ${advert.model}) - Please verify manually at: https://www.listingsitea.ch/de/d/${advert.listingsitea_id}`;
            ...
            const noLongerAvailable = `❌ [Swiss Individual] Advert ${advert.listingsitea_id} (${advert.make} ${advert.model}) no longer available`;
            ...
            const remainsInactive = `⏸️ [Swiss Individual] Advert ${advert.listingsitea_id} (${advert.make} ${advert.model}) remains inactive`;
        ...
        if (error.message && error.message.includes('Rate limit error')) {
          const rateLimitError = `⚠️ [Swiss Individual] Rate limit error for advert ${advert.listingsitea_id} - skipping to avoid false negative`;
          ...
          const generalError = `❌ Error checking Swiss advert ${advert.listingsitea_id}: ${error.message}`;
...
async function checkSwissDealerListings(user) {
  try {
    logger.info(`[CHECKER] 🇨🇭 Starting Swiss listings check for user ${user.id}: ${user.listingsitea_url}`);
    
    // Extract dealer ID from Swiss URL
    const dealerId = extractDealerIdFromChUrl(user.listingsitea_url);
    if (!dealerId) {
      throw new Error(`Could not extract dealer ID from Swiss URL: ${user.listingsitea_url}`);
    }
    ...
    const activeAdverts = await Advert.findAll({
      ...
      attributes: ['id', 'listingsitea_id', 'make', 'model', 'price', 'created_at']
    });
    logger.info(`[CHECKER] 💾 Database has ${activeAdverts.length} active adverts for user ${user.id}`);
    logger.info(`[CHECKER] 💾 Database advert IDs: [${activeAdverts.map(advert => advert.listingsitea_id).sort().join(', ')}]`);
    ...
    for (const advert of activeAdverts) {
      try {
        logger.info(`[CHECKER] 🔍 Checking advert ${advert.listingsitea_id} (type: ${typeof advert.listingsitea_id}) - exists in API: ${currentListingIds.has(advert.listingsitea_id)}`);
        if (!currentListingIds.has(advert.listingsitea_id)) {
          logger.warn(`[CHECKER] 🔍 Advert ${advert.listingsitea_id} NOT FOUND in API. API has: [${Array.from(currentListingIds).slice(0, 5).join(', ')}...]`);
        }
        ...
        if (currentListingIds.has(advert.listingsitea_id)) {
          ...
          logger.info(`[CHECKER] ✅ [Swiss] Advert ${advert.listingsitea_id} (${advert.make} ${advert.model}) still available`);
        } else {
          results.noLongerAvailable.push({
            listingsitea_id: advert.listingsitea_id,
            make: advert.make,
          ...
          logger.warn(`[CHECKER] ❌ [Swiss] Advert ${advert.listingsitea_id} (${advert.make} ${advert.model}) no longer available`);
          
          // Mark as inactive
          await markSwissAdvertAsInactive(advert);
        }
      } catch (error) {
        logger.error(`[CHECKER] ❌ Error checking Swiss advert ${advert.listingsitea_id}:`, error.message);
        results.errors.push({
          listingsitea_id: advert.listingsitea_id,
          error: error.message
        });
    ...
    const databaseListingIds = new Set(activeAdverts.map(advert => advert.listingsitea_id));
...
async function reactivateSwissAdvert(advert) {
  try {
    ...
    logger.info(`[CHECKER] ✅ [Swiss] Reactivated advert ${advert.listingsitea_id} - back online!`);
    
  } catch (error) {
    logger.error(`[CHECKER] ❌ Error reactivating Swiss advert ${advert.listingsitea_id}:`, error.message);
    throw error;
  }
}
...
async function markSwissAdvertAsInactive(advert) {
  try {
    ...
    if (isNaN(daysBetween) || !Number.isInteger(daysBetween) || daysBetween < 1) {
      logger.warn(`[CHECKER] ⚠️ [Swiss] Invalid daysBetween calculated for advert ${advert.listingsitea_id}, using 1 instead. created_at: ${advert.created_at}`);
      daysBetween = 1;
    }
    ...
    logger.info(`[CHECKER] ✅ [Swiss] Marked advert ${advert.listingsitea_id} as inactive (sold after ${daysBetween} days)`);
    
  } catch (error) {
    logger.error(`[CHECKER] ❌ Error marking Swiss advert ${advert.listingsitea_id} as inactive:`, error.message);
    throw error;
  }
}
...
function shouldUseSwissChecker(user) {
  return user.listingsitea_url && isSwissRegionUrl(user.listingsitea_url);
}
...
function logSwissCheckerResults(result) {
  ...
  if (result.stillAvailable.length > 0) {
    ...
    result.stillAvailable.forEach(advert => {
      logger.info(`[CHECKER]       - ${advert.listingsitea_id}: ${advert.make} ${advert.model} (${advert.price})`);
    });
  }
  ...
  if (result.noLongerAvailable.length > 0) {
    ...
    result.noLongerAvailable.forEach(advert => {
      logger.warn(`[CHECKER]       - ${advert.listingsitea_id}: ${advert.make} ${advert.model} (${advert.price})`);
    });
  }
  ...
  if (result.errors.length > 0) {
    ...
    result.errors.forEach(error => {
      logger.error(`[CHECKER]       - ${error.listingsitea_id}: ${error.error}`);
    });
  }
}
