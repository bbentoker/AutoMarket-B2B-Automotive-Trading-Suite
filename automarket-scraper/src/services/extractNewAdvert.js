...
async function getListingInfos(advertUrl, advertId, user, isInitialRun = false) {
  try {
    ...
    const response = await axios.get(advertUrl, { httpsAgent: getRequestAgent() });
...
async function extractNewAdvert(advertUrl, advertId, user, isInitialRun = false) {
  try {
    ...
    // Check for existing listing with same characteristics (excluding listingsitea_id and image URLs)
    ...
    if (existingListing) {
      console.log(`[SCRAPER] Found existing listing with ID: ${existingListing.id}. Updating listingsitea_id and reactivating.`);
      
      // Update the existing listing with new listingsitea_id and image URLs, and reactivate it
      await existingListing.update({
        listingsitea_id: extractedData.listingsitea_id,
      ...
      console.log(`[SCRAPER] Updated existing listing with new listingsitea_id: ${extractedData.listingsitea_id}`);
