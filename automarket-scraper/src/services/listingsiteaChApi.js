...
/**
 * ListingSiteA.ch (Swiss) API Service
 * Implementation based on listingsitea_ch_docs.md specifications
 */

// Constants from documentation
const API_BASE_URL = 'https://api.listingsitea.ch';
const WEBSITE_BASE_URL = 'https://www.listingsitea.ch';
...
/**
 * Extract dealer ID from Swiss ListingSiteA URL
 * Example: https://www.listingsitea.ch/de/s/seller-1729890 -> 1729890
 */
...
/**
 * Check if URL is from Swiss ListingSiteA region
 */
function isSwissRegionUrl(url) {
  return url && url.includes('listingsitea.ch');
}

/**
 * Get required headers for Swiss ListingSiteA API
...
/**
 * Fetch dealer listings from Swiss ListingSiteA API
 */
...
async function fetchSwissListingById(listingId, debugLogFilePath = null) {
  const url = `https://api.listingsitea.ch/v1/listings/${listingId}`;
  const headers = getSwissApiHeaders();
