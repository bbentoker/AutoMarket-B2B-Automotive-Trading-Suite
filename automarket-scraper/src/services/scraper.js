...
const { Advert, Control, ListingSiteAInventory } = require('../../models');
const {
  ...
  getHttpsAgent,
  getRequestAgent,
} = require('./listingsiteaApi');
const {
  isSwissRegionUrl,
  scrapeSwissDealer,
} = require('./listingsiteaChApi');

const advertBaseUrl = 'https://www.listingsitea.example.com/offers/';
...
// Utils moved to listingsiteaApi.js for readability

/**
 * Swiss region scraping using ListingSiteA.ch API
 */
async function searchAllPagesViaSwissApi(user, control, isInitialRun = false) {
  try {
    console.log(`[SCRAPER] 🇨🇭 Starting Swiss region scraping for user ${user.id}: ${user.listingsitea_url}`);
    
    // Use the Swiss API service to scrape dealer listings
    const result = await scrapeSwissDealer(user.listingsitea_url, user.id);
...
async function createSwissAdvert(listing, user, isInitialRun = false) {
  try {
    ...
    const originalImageUrl = firstImage ? `https://listing-images.listingsitea.ch/${firstImage.key}` : null;
    ...
    const advertData = {
      listingsitea_id: String(listing.id), // Convert to string as required by database schema
      seller_id: user.id,
      ...
      link: `https://www.listingsitea.ch/de/d/${listing.id}`,
...
async function processSwissListingsSequentially(listings, user, control, isInitialRun = false) {
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    try {
      ...
      // Convert to string for database comparison (listingsitea_id is STRING in schema)
      const articleIdStr = String(articleId);
      
      const existingAdvert = await Advert.findOne({
        where: {
          listingsitea_id: articleIdStr,
          seller_id: user.id
        }
      });
...
async function searchAllPagesViaApi(user, control, isInitialRun = false) {
  try {
    ...
    if (isSwissRegionUrl(user.listingsitea_url)) {
      console.log(`[SCRAPER] 🇨🇭 Detected Swiss region URL: ${user.listingsitea_url}`);
      return await searchAllPagesViaSwissApi(user, control, isInitialRun);
    }
    
    console.log(`[SCRAPER] 🇧🇪 Using Belgian region API for: ${user.listingsitea_url}`);
    ...
    const dealerRes = await fetchWith429Retry('dealer page', () => axios.get(user.listingsitea_url, {
    ...
    if (!customerId) {
      console.error('[SCRAPER] ❌ Could not resolve customerId from dealer page:', user.listingsitea_url);
      return;
    }
    const cultureIso = resolveCultureIsoFromUrl(user.listingsitea_url);
...
async function searchAllPages(user, control) {
    try {
      const response = await axios.get(user.listingsitea_url, { httpsAgent: getRequestAgent() });
      ...
      console.log(`[SCRAPER] 📄 Total pages found: ${totalPages}`);
        ...
        try {
          ...
          const pageUrl = user.listingsitea_url.includes('?') 
            ? `${user.listingsitea_url}&page=${page}` 
            : `${user.listingsitea_url}?page=${page}`;
          const pageResponse = await axios.get(pageUrl, { httpsAgent: getRequestAgent() });
 ...
 * @param {Object} user - The user object (must have listingsitea_url).
...
async function searchAllPagesWithAllSorts(user, control) {
  try {
    ...
    for (const sortOption of sortingOptions) {
      ...
      await searchAllPages({ ...user, listingsitea_url: url }, control);
    }
  } catch (error) {
