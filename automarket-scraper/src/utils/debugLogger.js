...
class DebugLogger {
    ...
    logUserStart(user, currentIndex, totalUsers) {
        const message = `🚀 STARTED processing user ${user.id} (${user.company_name || 'Unknown'}) - ${currentIndex}/${totalUsers} - URL: ${user.listingsitea_url}`;
    ...
    clearDebugFile() {
        try {
            const timestamp = new Date().toISOString();
            const header = `ListingSiteA Scraper Debug Log - Session started at ${timestamp}\n${'='.repeat(80)}\n`;
