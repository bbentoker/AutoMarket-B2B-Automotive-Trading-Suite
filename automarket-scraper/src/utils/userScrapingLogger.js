...
class UserScrapingLogger {
    ...
    /**
     * Check if user is Swiss based on listingsitea URL
     * @param {Object} user - User object with listingsitea_url
     * @returns {boolean} - True if user is Swiss (listingsitea.ch)
     */
    isSwissUser(user) {
        return user.listingsitea_url && user.listingsitea_url.includes('listingsitea.ch');
    }
    ...
    clearLogFile() {
        ...
        try {
            const timestamp = new Date().toISOString();
            const header = `ListingSiteA User Scraping Log - Session started at ${timestamp}\n`;
